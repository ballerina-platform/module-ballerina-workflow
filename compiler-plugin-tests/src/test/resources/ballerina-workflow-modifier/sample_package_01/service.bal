...
service "workflow" on new workflow:WorkflowEventListener(persistentProvider) {
    isolated remote function execute() {
        var a = performActivity1();
        var b = performActivity2(5, "john");
        var c = foo(3, "doe");
        var d = m1:performActivity(7, "smith");
        var e = {"a": "a"};
    }
}

@workflow:Activity
isolated function performActivity1() returns int {
    return 10;
}

@workflow:Activity
isolated function performActivity2(int a, string name) returns int {
    return a;
}
