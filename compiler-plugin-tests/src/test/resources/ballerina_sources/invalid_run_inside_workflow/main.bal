...
function parentProcess(workflow:Context ctx, ChildInput input) returns string|error {
    // ERROR: workflow:run is a client verb; inside a workflow use ctx->runChildWorkflow
    string childId = check workflow:run(childProcess, {value: "x"});
    // ERROR: workflow:sendData is a client verb; inside a workflow use ctx->sendDataToChildWorkflow
    check workflow:sendData(receiverProcess, childId, "notification", {"message": "hi"});
    return childId;
}
