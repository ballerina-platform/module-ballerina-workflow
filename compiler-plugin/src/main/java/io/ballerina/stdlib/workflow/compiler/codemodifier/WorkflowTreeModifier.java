...
public class WorkflowTreeModifier extends TreeModifier {
    ...
    public FunctionDefinitionNode transform(FunctionDefinitionNode functionDefinitionNode) {
        // execute modification only for the 'execute' function
        if (functionDefinitionNode.functionName().text().equals(EXECUTE)) {
    ...
    public FunctionCallExpressionNode transform(FunctionCallExpressionNode functionCallNode) {
        ...
        if (activityFunctionSym != null && functionCallNode.parent().kind() == SyntaxKind.LOCAL_VAR_DECL) {
            ...
            List<Node> arrayMembers = new ArrayList<>();
            if (!argList.isEmpty()) {
                int size = argList.size();
                for (int i = 0; i < size; i++) {
                    arrayMembers.add(argList.get(i));
                    if (i < size - 1) {
                        arrayMembers.add(createToken(SyntaxKind.COMMA_TOKEN));
                    }
                }
                newArgs.add(createToken(SyntaxKind.COMMA_TOKEN));
                newArgs.add(NodeFactory.createListConstructorExpressionNode(
                        createToken(SyntaxKind.OPEN_BRACKET_TOKEN),
                        AbstractNodeFactory.createSeparatedNodeList(arrayMembers),
                        createToken(SyntaxKind.CLOSE_BRACKET_TOKEN)
                ));
            }
            SeparatedNodeList<FunctionArgumentNode> arguments = AbstractNodeFactory.createSeparatedNodeList(newArgs);
            functionCallNode = functionCallNode.modify().withFunctionName(newFuncName).withArguments(arguments).apply();
        }
